...
const obtenerOpciones = (req, res) => {
  ...
  con.query(sql, function(error, resultadoCompetencia, fields) {
    ...
    let queryPeliculas =
      "SELECT DISTINCT pelicula.id, poster, titulo, genero_id FROM pelicula LEFT JOIN actor_pelicula ON pelicula.id = actor_pelicula.pelicula_id LEFT JOIN director_pelicula ON pelicula.id = director_pelicula.pelicula_id WHERE 1=1";
    ...
    let queryDirector = director
      ? ` AND director_pelicula.director_id = ${director}`
      : ""; // Si se utiliza un director como filtro, se agrega a la query. //
      
    let randomOrder = ` ORDER BY RAND() LIMIT 2;`; // Se agrega filtro para que devuelva dos películas al azar. //

    let sql2 =
      queryPeliculas + queryGenero + queryActor + queryDirector + randomOrder; // Se arma la query final con los filtros que se hayan seleccionado. //
